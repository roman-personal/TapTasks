...
namespace TapTasks {
    public partial class Form1 : Form {
        ...
        private async void Button2_Click(object sender, EventArgs e) {
            sw.Restart();
            int count = await GetLinesCount2Async(fileName);
            sw.Stop();
            label1.Text = $"Lines count: {count}, elapsed: {sw.Elapsed}";
        }
        // Открытие потока с большим буфером и в асинхронном режиме - лучше с производительностью
        private async Task<int> GetLinesCount2Async(string fileName) {
            if (string.IsNullOrEmpty(fileName))
                throw new ArgumentException("Parameter should not be null or empty!", nameof(fileName));
        ...
        private async void Button3_Click(object sender, EventArgs e) {
            sw.Restart();
            int count = await GetLinesCount3Async(fileName);
            sw.Stop();
            label1.Text = $"Lines count: {count}, elapsed: {sw.Elapsed}";
        }
        // Совсем хорошо с производительностью, но с синхронным чтением и в отдельном потоке
        private Task<int> GetLinesCount3Async(string fileName) {
            if (string.IsNullOrEmpty(fileName))
                throw new ArgumentException("Parameter should not be null or empty!", nameof(fileName));
            return Task.Run(() => {
